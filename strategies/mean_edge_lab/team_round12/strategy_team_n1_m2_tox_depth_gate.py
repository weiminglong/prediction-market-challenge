"""m2 calm L2/L3 depth + lightweight depth-tox memory (adverse move, one-sided fills).

Same j3 regime machine and calm-depth eligibility as m2; toxicity only scales or zeros
passive L3 then L2 sizes—L1 quotes unchanged until existing regime/spread logic applies.
"""
...
class Strategy(BaseStrategy):
    ...
    # Depth-only tox EWMA (does not replace fill_bias in regime)
    _D_TOX_DECAY = 0.895
    ...
    def _depth_layer_scales(self) -> tuple[float, float]:
        """Return (l2_scale, l3_scale); L3 is reduced first as tox_depth_mem rises."""
        t = self.tox_depth_mem
        l3 = max(0.0, min(1.0, 1.0 - max(0.0, t - 0.52) / 1.32))
        l2 = max(0.0, min(1.0, 1.0 - max(0.0, t - 0.88) / 1.42))
        return l2, l3
