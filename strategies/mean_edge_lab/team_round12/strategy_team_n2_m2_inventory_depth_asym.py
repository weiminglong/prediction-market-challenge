"""m2 j3 calm-depth with L2/L3 size and offset skew toward inventory reduction."""
...
class Strategy(BaseStrategy):
    ...
    _L3_FRAC = 0.21
    _MIN_DEPTH = 0.5
    # Calm-depth inventory asymmetry (mild; capped)
    ...
    def _depth_inv_skew(self, net_inv: float) -> tuple[float, float, float, float]:
        """Returns (bid_size_mult, ask_size_mult, bid_off_mult, ask_off_mult)."""
        t = math.tanh(net_inv / self._DEPTH_INV_TANH_SCALE)
        c = self._DEPTH_SIZE_COEF
        bid_sz = max(0.78, min(1.22, 1.0 - c * t))
        ask_sz = max(0.78, min(1.22, 1.0 + c * t))
        o = self._DEPTH_OFF_COEF
        bid_off = max(0.82, min(1.18, 1.0 + o * t))
        ask_off = max(0.82, min(1.18, 1.0 - o * t))
    ...
    def on_step(self, state: StepState):
        ...
        bsz_m, asz_m, boff_m, aoff_m = self._depth_inv_skew(net_inv)
