"""m2 + adaptive cash reserve and depth throttle from realized variance (EMA of squared mid moves)."""
...
class Strategy(BaseStrategy):
    ...
    _L3_FRAC = 0.21
    _MIN_DEPTH = 0.5
    # Realized variance: EMA on squared mid move; stress in [0, ~1.2] drives reserve + depth
    _RV_EMA_BETA = 0.07
    _STRESS_DIV = 2.35
    _STRESS_CAP = 1.2
    _RESERVE_BASE = 4.85
    _RESERVE_STRESS = 2.35
    _DEPTH_VOL_CALM = 0.09
    _DEPTH_VOL_STRESS = 0.14
    _DEPTH_FRAC_CALM = 0.06
    _DEPTH_EXTRA_STRESS = 0.20
    ...
    def _rv_stress(self) -> float:
        rv_t = math.sqrt(max(0.0, self.rv_ema))
        return min(self._STRESS_CAP, rv_t / max(self._STRESS_DIV, 1e-9))

    def _depth_allowed(self, risk_off: bool, shock_active: bool, regime_raw: float, vol_cap_eff: float) -> bool:
        if risk_off or shock_active:
            return False
        if regime_raw > self._DEPTH_REGIME_MAX:
            return False
        if self.vol_ema >= vol_cap_eff:
            return False
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            b = self._RV_EMA_BETA
            self.rv_ema = (1.0 - b) * self.rv_ema + b * (move * move)
        ...
        stress = self._rv_stress()
        stress_u = stress / max(self._STRESS_CAP, 1e-9)
        ...
        reserve_mult = self._RESERVE_BASE + self._RESERVE_STRESS * stress
        spendable = max(0.0, state.free_cash - reserve_mult * vol_ref)
        ...
        calm = 1.0 if stress < 0.14 else 0.0
        vol_cap_eff = self._DEPTH_VOL_MAX + self._DEPTH_VOL_CALM * calm - self._DEPTH_VOL_STRESS * stress_u
        vol_cap_eff = max(0.40, vol_cap_eff)
        depth_ok = self._depth_allowed(risk_off, shock_active, regime_raw, vol_cap_eff)

        frac_adj = 1.0 + self._DEPTH_FRAC_CALM * calm - 0.12 * stress_u
        frac_adj = max(0.82, min(1.08, frac_adj))
        extra_scale = 1.0 - self._DEPTH_EXTRA_STRESS * stress_u
        extra_scale = max(0.72, extra_scale)
        ...
        l2e = max(2, int(round(self._L2_EXTRA * extra_scale)))
        l3e = max(3, int(round(self._L3_EXTRA * extra_scale)))
        ms = self._MIN_DEPTH
        l2f = self._L2_FRAC * frac_adj
        l3f = self._L3_FRAC * frac_adj

        bid_L2 = max(1, my_bid - l2e)
        ask_L2 = min(99, my_ask + l2e)
        bid_sz2 = max(ms, bid_size * l2f) if bid_size > 0 else 0.0
        ask_sz2 = max(ms, ask_size * l2f) if ask_size > 0 else 0.0
        ...
        bid_sz3 = max(ms, bid_size * l3f) if bid_size > 0 else 0.0
        ask_sz3 = max(ms, ask_size * l3f) if ask_size > 0 else 0.0
