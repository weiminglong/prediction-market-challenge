"""m2 calm L2/L3 + depth-only tox EWMA with enter/exit hysteresis (L2/L3 only).

Same j3 regime machine and calm-depth eligibility as m2; a separate tox_depth_mem
tracks adverse post-fill mid moves. Hysteresis avoids flicker: elevated mode turns on
at a higher mem threshold and off at a lower one; in elevated mode L3 is off and L2
sizes are reduced. L1 quotes unchanged by this path.
"""
...
class Strategy(BaseStrategy):
    ...
    # Hysteresis on tox_depth_mem: enter elevated L2/L3 pull at high, exit at low.
    _TOX_H_ENTER = 1.02
    _TOX_H_EXIT = 0.58
    _TOX_H_L2_FRAC = 0.38  # fraction of normal L2 size while elevated (L3 -> 0)
    ...
    def on_step(self, state: StepState):
        ...
        l2_scale, l3_scale = self._depth_layer_scales()
        ...
        f2 = self._L2_FRAC * l2_scale
        bid_sz2 = max(ms, bid_size * f2) if bid_size > 0 and l2_scale > 0 else 0.0
        ask_sz2 = max(ms, ask_size * f2) if ask_size > 0 and l2_scale > 0 else 0.0
        ...
        f3 = self._L3_FRAC * l3_scale
        bid_sz3 = max(ms, bid_size * f3) if bid_size > 0 and l3_scale > 0 else 0.0
        ask_sz3 = max(ms, ask_size * f3) if ask_size > 0 and l3_scale > 0 else 0.0
