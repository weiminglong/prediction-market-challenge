"""G3 round5: deeper convex inventory skew by depth tiers + asymmetric size caps under collateral stress."""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # Depth tiers: amplify convex skew when |inv| crosses fraction of max_inventory
        "depth_tier1": 0.42,
        "depth_tier2": 0.78,
        "depth_amp1": 1.12,
        "depth_amp2": 1.38,
        "depth_amp3": 1.72,
        "depth_curve_pow": 2.35,
        # Collateral stress: scale sizes asymmetrically (0 = off, 1 = full effect)
        "stress_cash_ref": 28.0,
        "stress_soft": 14.0,
        "stress_bid_long": 0.42,
        "stress_ask_long_uncov": 0.55,
        "stress_bid_short": 0.28,
        "stress_ask_short": 0.18,
    }
    ...
    def _raw_convex_inv_shift(self, net_inv: float) -> float:
        p = self.PARAMS
        if net_inv == 0.0:
            return 0.0
    ...
    def _depth_tier_multiplier(self, abs_inv: float) -> float:
        p = self.PARAMS
        max_inv = max(float(p["max_inventory"]), 1e-6)
        u = min(1.0, abs_inv / max_inv)
        t1 = min(max(float(p["depth_tier1"]), 0.05), 0.95)
        t2 = min(max(float(p["depth_tier2"]), t1 + 0.05), 0.99)
        a1 = float(p["depth_amp1"])
        a2 = float(p["depth_amp2"])
        a3 = float(p["depth_amp3"])
        pw = max(1.0, float(p["depth_curve_pow"]))
        if u <= t1:
            return a1
        if u <= t2:
            span = max(t2 - t1, 1e-9)
            x = (u - t1) / span
            return a1 + (a2 - a1) * (x**pw)
        span = max(1.0 - t2, 1e-9)
        x = (u - t2) / span
        return a2 + (a3 - a2) * (x**pw)

    def _convex_inv_shift(self, net_inv: float) -> float:
        if net_inv == 0.0:
            return 0.0
        m = self._depth_tier_multiplier(abs(net_inv))
        return self._raw_convex_inv_shift(net_inv) * m

    def _collateral_stress01(self, free_cash: float) -> float:
        p = self.PARAMS
        ref = max(1.0, float(p["stress_cash_ref"]))
        soft = max(0.0, float(p["stress_soft"]))
        ratio = free_cash / ref
        if ratio >= 1.0:
            return 0.0
        hi = 1.0
        lo = max(0.0, (soft / ref) if ref > 0 else 0.0)
        if ratio >= hi:
            return 0.0
        if ratio <= lo:
            return 1.0
        return (hi - ratio) / max(hi - lo, 1e-9)

    def _apply_stress_asymmetric_caps(
        self,
        net_inv: float,
        bid_size: float,
        ask_size: float,
        ...
        unc_frac = 0.0
        if ask_size > 1e-12:
            unc_frac = max(0.0, min(1.0, (ask_size - avail_yes) / ask_size))

        if net_inv > 0.0:
            bid_size *= max(0.0, 1.0 - stress * float(p["stress_bid_long"]))
            ask_size *= max(0.0, 1.0 - stress * float(p["stress_ask_long_uncov"]) * unc_frac)
        elif net_inv < 0.0:
            bid_size *= max(0.0, 1.0 - stress * float(p["stress_bid_short"]))
            ask_size *= max(0.0, 1.0 - stress * float(p["stress_ask_short"]))
        else:
            bid_size *= max(0.0, 1.0 - stress * 0.5 * (float(p["stress_bid_long"]) + float(p["stress_bid_short"])))
            ask_size *= max(0.0, 1.0 - stress * 0.5 * (float(p["stress_ask_long_uncov"]) + float(p["stress_ask_short"])))
    ...
    def on_step(self, state: StepState):
        ...
        stress = self._collateral_stress01(state.free_cash)
        bid_size, ask_size = self._apply_stress_asymmetric_caps(
            net_inv, bid_size, ask_size, avail_yes, stress
        )
