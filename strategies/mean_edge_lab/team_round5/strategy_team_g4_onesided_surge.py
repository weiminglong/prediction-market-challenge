"""G4 round 5: one-sided surge when trend and shock align; slow toxic-side re-entry."""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # G4: require |trend| above this (in tick units) to treat as aligned with shock
        "trend_align_eps": 0.08,
        # Extra size multiplier on the favorable leg during aligned surge
        "surge_favorable_boost": 1.55,
        # Pull favorable quote this many half-spread ticks closer to fair (min half stays 1)
        "surge_half_spread_clip": 1,
        # Post–aligned-shock: ramp suppressed (toxic) side from floor to full over this many steps
        "toxic_reentry_steps": 6,
        "toxic_reentry_floor": 0.18,
    }
    ...
    def __init__(self):
        ...
        self.shock_sign = 0
        self._toxic_reentry_left = 0
        self._toxic_side: str | None = None
        self._aligned_surge_last_step = False
    ...
    def on_step(self, state: StepState):
        ...
        eps = float(p["trend_align_eps"])
        aligned_surge = self.shock_remaining > 0 and (
            (self.trend > eps and self.shock_sign > 0) or (self.trend < -eps and self.shock_sign < 0)
        )
        self._aligned_surge_last_step = aligned_surge

        half_bid = half_spread
        half_ask = half_spread
        if aligned_surge:
            clip = int(p["surge_half_spread_clip"])
            if self.shock_sign > 0:
                half_ask = max(1, half_spread - clip)
            else:
                half_bid = max(1, half_spread - clip)

        my_bid = max(1, int(round(fair - half_bid)))
        my_ask = min(99, int(round(fair + half_ask)))
        if my_bid >= my_ask:
            return actions
        ...
        boost = float(p["surge_favorable_boost"])
        shock_before = self.shock_remaining
        if shock_before > 0:
            if aligned_surge:
                if self.shock_sign > 0:
                    bid_size = 0.0
        ...
        if shock_before > 0 and self.shock_remaining == 0 and self._aligned_surge_last_step:
            n = max(1, int(p["toxic_reentry_steps"]))
            self._toxic_reentry_left = n
            self._toxic_side = "bid" if self.shock_sign > 0 else "ask"

        # Ramp begins first step after shock ends (not the same step we arm it).
        if self._toxic_reentry_left > 0 and self._toxic_side is not None and shock_before == 0:
            n = max(1, int(p["toxic_reentry_steps"]))
            k = n - self._toxic_reentry_left + 1
            floor_m = float(p["toxic_reentry_floor"])
            toxic_mult = floor_m + (1.0 - floor_m) * min(1.0, k / float(n))
            if self._toxic_side == "bid":
                bid_size *= toxic_mult
            else:
                ask_size *= toxic_mult
            self._toxic_reentry_left -= 1
            if self._toxic_reentry_left <= 0:
                self._toxic_side = None
