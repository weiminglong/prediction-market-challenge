"""M4: multilevel quoting with state-aware inside-spread sentinel gating.

Same stack as strat2_team/strategy_multilevel.py; sentinel block only is extended with:
- Quiet streak +1 when vol_estimate exceeds the same threshold used for quote widening
- Spread-quality band (min/max competitor ticks) to skip absurd books
- Cooldown after arb-like one-sided flow
- Same-step adverse-fill guard: fills paired with a large mid jump against the fill
All thresholds are fixed constants (no seed-based tuning).
"""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # Sentinel filter (fixed; not seed-tuned)
        "sentinel_spread_max": 55,
        "sentinel_cooldown_steps": 3,
        "sentinel_adverse_step": 1.28,
    }
    ...
    def on_step(self, state: StepState):
        ...
        # Track quiet periods (same reset rules as baseline multilevel)
        ...
        ast = float(p["sentinel_adverse_step"])
        adverse_step = (buy_qty > 0.5 and move <= -ast) or (sell_qty > 0.5 and move >= ast)

        if arb_like:
            self.sentinel_cooldown = max(self.sentinel_cooldown, int(p["sentinel_cooldown_steps"]))
        ...
        # Sentinel orders: vol-linked quiet streak, arb cooldown, optional spread ceiling
        comp_spread = ask - bid
        extra_quiet = 1 if self.vol_estimate > float(p["spread_vol_threshold"]) else 0
        need_quiet = int(p["sentinel_quiet_steps"]) + extra_quiet
        spread_ok = int(p["sentinel_min_spread"]) <= comp_spread <= int(p["sentinel_spread_max"])
        cd_ok = self.sentinel_cooldown == 0

        if (
            spread_ok
            and self.quiet_steps >= need_quiet
            and self.shock_remaining <= 0
            and cd_ok
            and not adverse_step
        ):
