"""J3 regime switch + realized-variance risk budget for sizing (risk-on scales up when calm)."""
...
class Strategy(BaseStrategy):
    ...
    # --- Realized variance budget (few params; L3-style) ---
    _RV_ALPHA = 0.08
    ...
    _VAR_RISK_MIN = 0.42
    _VAR_RISK_MAX = 1.30
    _VAR_THROTTLE_SIG = 1.02
    _VAR_THROTTLE_SIZE = 0.83
    ...
    def on_step(self, state: StepState):
        ...
        if risk_off:
            rv_risk = min(1.0, var_budget)
        else:
            rv_risk = var_budget
        if sig_rv >= self._VAR_THROTTLE_SIG:
            rv_risk *= self._VAR_THROTTLE_SIZE

        max_inv = 8.7
        bid_size = max(0.5, base_size * vol_scale * rv_risk * max(0.0, 1.0 - net_inv / max_inv))
        ask_size = max(0.5, base_size * vol_scale * rv_risk * max(0.0, 1.0 + net_inv / max_inv))
