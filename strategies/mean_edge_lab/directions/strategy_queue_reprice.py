"""Direction B: queue-preserving selective replace market maker."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        self.momentum = 0.65 * self.momentum + 0.35 * move
        ...
        fair = mid + self.fill_bias + self.momentum * 0.35 - net_inventory * 0.065

        bid_spread = 2 + (1 if self.bid_cooldown > 0 else 0)
        ask_spread = 2 + (1 if self.ask_cooldown > 0 else 0)
        if self.vol_estimate > 1.6:
            bid_spread += 1
            ask_spread += 1
        ...
        vol_scale = max(0.25, 1.0 - self.vol_estimate * 0.55)
        base_size = 1.05 * vol_scale
        bid_size = max(0.15, base_size * max(0.0, 1.0 - net_inventory / 32.0))
        ask_size = max(0.15, base_size * max(0.0, 1.0 + net_inventory / 32.0))
        if net_inventory >= 44.0:
            bid_size = 0.0
        if net_inventory <= -44.0:
            ask_size = 0.0
