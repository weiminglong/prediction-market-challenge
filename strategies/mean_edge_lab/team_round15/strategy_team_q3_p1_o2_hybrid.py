"""p1 o4 tox×trend depth + o2-style tox_depth_mem hysteresis for L2/L3 only (L1 unchanged)."""
...
class Strategy(BaseStrategy):
    ...
    # o2 depth-only tox EWMA + hysteresis (L2/L3 gates only)
    _D_TOX_DECAY = 0.895
    _D_TOX_VOL_FLOOR = 0.07
    _D_TOX_ADVERSE_W = 0.48
    _D_TOX_ONESIDED_W = 0.22
    _D_TOX_ONESIDED_RATIO = 1.82
    _D_TOX_MIN_FILL = 0.28
    _D_TOX_ADV_FOR_ONESIDED = 0.085
    _D_TOX_MIN_ABS_MOVE = 0.38
    _D_TOX_STEP_CAP = 0.88
    _D_TOX_MEM_CAP = 2.95
    _TOX_H_ENTER = 1.02
    _TOX_H_EXIT = 0.58
    _TOX_H_L2_FRAC = 0.38
    ...
    def __init__(self):
        ...
        self.tox_depth_mem = 0.0
        self.tox_depth_elevated = False
    ...
    def _update_depth_tox(self, move: float, buy_qty: float, sell_qty: float) -> None:
        self.tox_depth_mem = min(self._D_TOX_MEM_CAP, self.tox_depth_mem * self._D_TOX_DECAY)
        if abs(move) < self._D_TOX_MIN_ABS_MOVE:
            self._update_tox_hysteresis()
            return
        vd = max(self.vol_ema, self._D_TOX_VOL_FLOOR, abs(move) + 1e-9)
        inst = 0.0
        ratio = self._D_TOX_ONESIDED_RATIO

        if buy_qty >= self._D_TOX_MIN_FILL:
            adv_b = max(0.0, -move) / vd
            inst += self._D_TOX_ADVERSE_W * adv_b
            if sell_qty * ratio < buy_qty and adv_b >= self._D_TOX_ADV_FOR_ONESIDED:
                inst += self._D_TOX_ONESIDED_W * min(buy_qty / 8.0, 1.0)

        if sell_qty >= self._D_TOX_MIN_FILL:
            adv_s = max(0.0, move) / vd
            inst += self._D_TOX_ADVERSE_W * adv_s
            if buy_qty * ratio < sell_qty and adv_s >= self._D_TOX_ADV_FOR_ONESIDED:
                inst += self._D_TOX_ONESIDED_W * min(sell_qty / 8.0, 1.0)

        self.tox_depth_mem = min(self._D_TOX_MEM_CAP, self.tox_depth_mem + min(self._D_TOX_STEP_CAP, inst))
        self._update_tox_hysteresis()

    def _update_tox_hysteresis(self) -> None:
        m = self.tox_depth_mem
        if not self.tox_depth_elevated:
            if m >= self._TOX_H_ENTER:
                self.tox_depth_elevated = True
        elif m <= self._TOX_H_EXIT:
            self.tox_depth_elevated = False

    def _depth_layer_scales(self) -> tuple[float, float]:
        if not self.tox_depth_elevated:
            return 1.0, 1.0
        return self._TOX_H_L2_FRAC, 0.0
    ...
    def on_step(self, state: StepState):
        ...
        self._update_depth_tox(move, state.buy_filled_quantity, state.sell_filled_quantity)
        ...
        l2h, l3h = self._depth_layer_scales()
        ...
        bid_sz2 = base_b2 * l23_b_scale * l2h
        ask_sz2 = base_a2 * l23_a_scale * l2h
        if bid_sz2 < ms:
            bid_sz2 = 0.0
        if ask_sz2 < ms:
            ask_sz2 = 0.0
        ...
        bid_sz3 = base_b3 * l23_b_scale * l3h
        ask_sz3 = base_a3 * l23_a_scale * l3h
        if bid_sz3 < ms:
            bid_sz3 = 0.0
        if ask_sz3 < ms:
            ask_sz3 = 0.0
