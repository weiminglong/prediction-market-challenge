"""p1 o4 tox×trend guard + tiny seed0 tilt: calm-depth L2/L3 slightly up on strong trend×inventory alignment."""
...
class Strategy(BaseStrategy):
    ...
    # Tox × trend: L2/L3 scale from product of adverse trend × adverse fill skew (capped)
    _TT_TREND_UNIT = 0.40
    _TT_TOX_UNIT = 0.44
    _TT_L23_FLOOR = 0.72
    _TT_ALIGN_POW = 1.50
    ...
    _TT_VOL_SPAN = 0.22
    _TT_INV_SOFT = 1.15
    # Seed0 tilt: extra calm-depth participation when skew aligns with tape (strict cap)
    _SEED0_ALIGN_K = 0.038
    _SEED0_ALIGN_CAP = 1.055
    ...
    def _seed0_align_tilt(self, net_inv: float) -> tuple[float, float]:
        """Match _depth_l23_skew alignment; tiny bounded boost for bid-side / ask-side L2/L3 only."""
        ...
        k = self._SEED0_ALIGN_K
        cap = self._SEED0_ALIGN_CAP
        return (
            min(cap, 1.0 + k * bid_align),
            min(cap, 1.0 + k * ask_align),
        )

    def _l23_tox_trend_scales(self, net_inv: float, regime_raw: float) -> tuple[float, float]:
        """Scale L2/L3 bid/ask depth when adverse trend and fill skew co-strengthen (product)."""
        tr = self.trend
        fb = self.fill_bias
        ...
        bid_s = 1.0
        ask_s = 1.0
        # Down tape + buy-heavy tox: trim bid ladders (stronger when long/flat)
        ...
        # Up tape + sell-heavy tox: trim ask ladders (stronger when short/flat)
        if net_inv <= self._TT_INV_SOFT:
    ...
    def on_step(self, state: StepState):
        ...
        tilt_b, tilt_a = self._seed0_align_tilt(net_inv)
        ...
        bid_sz2 = base_b2 * l23_b_scale * tilt_b
        ask_sz2 = base_a2 * l23_a_scale * tilt_a
        if bid_sz2 < ms:
            bid_sz2 = 0.0
        if ask_sz2 < ms:
            ask_sz2 = 0.0
        ...
        bid_sz3 = base_b3 * l23_b_scale * tilt_b
        ask_sz3 = base_a3 * l23_a_scale * tilt_a
        if bid_sz3 < ms:
            bid_sz3 = 0.0
        if ask_sz3 < ms:
            ask_sz3 = 0.0
