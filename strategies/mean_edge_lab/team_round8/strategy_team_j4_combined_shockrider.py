"""Combined MM + shock one-side, with trend-aligned shock surge and reversal brake."""
...
class Strategy(BaseStrategy):
    def __init__(self):
        ...
        self.shock_sign = 0
        self.shock_aligned = False
        self.reversal_brake = 0
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            self.trend = 0.66 * self.trend + 0.165 * move
            shock_trigger = max(1.86, 3.02 * max(self.vol_ema, 0.34))
            rev_th = 0.74 * shock_trigger
            if abs(move) >= rev_th and move * trend_before < 0.0 and abs(trend_before) >= 0.16:
                self.reversal_brake = 5
            if abs(move) >= shock_trigger:
                self.shock_remaining = 4
                self.shock_sign = 1 if move > 0 else -1
                self.shock_aligned = (
                    self.shock_sign * trend_before > 0.0 and abs(trend_before) >= 0.21
                )
        self.prev_bid = bid_t
        self.prev_ask = ask_t
        ...
        if self.reversal_brake > 0:
            half_spread += 1

        half_b = half_spread
        half_a = half_spread
        if self.shock_remaining > 0 and self.shock_aligned and self.reversal_brake == 0:
            if self.shock_sign > 0:
                half_b = max(1, half_spread - 1)
            elif self.shock_sign < 0:
                half_a = max(1, half_spread - 1)

        my_bid = max(1, int(round(fair - half_b)))
        my_ask = min(99, int(round(fair + half_a)))
        if my_bid >= my_ask:
            return actions

        model_boost = max(0.5, min(3.0, 1.0 / max(0.3, model_vol)))
        base_size = 10.0 * model_boost
        vol_scale = max(0.06, 1.0 - self.vol_ema * 2.35)
        shock_floor = 0.15
        if self.shock_remaining > 0:
            if self.shock_aligned and self.reversal_brake == 0:
                shock_floor = 0.34
            vol_scale *= shock_floor
