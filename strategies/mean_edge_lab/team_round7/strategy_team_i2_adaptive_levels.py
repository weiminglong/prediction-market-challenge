"""I2: multilevel quoting with adaptive depth from regime quality.

Calm / benign → up to four ladder levels and a larger inside-spread sentinel.
Volatile, shock, or one-sided toxic fills → collapse to fewer levels, smaller
quotes, and higher cash reserve drag. Core cash/collateral checks match the
multilevel baseline.
"""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # Multi-level ladder (same as baseline)
        "l2_spread_extra": 5,
        "l2_size_frac": 0.5,
        "l3_spread_extra": 9,
        "l3_size_frac": 0.3,
        ...
        # Adaptive depth: vol_estimate thresholds (ticks/step noise scale)
        "depth_vol_three": 0.48,
        "depth_vol_two": 0.84,
        "depth_vol_one": 1.12,
        "toxic_cooldown_steps": 3,
        "toxic_max_levels": 3,
        "stress_size_mult_12": 0.91,
        "stress_size_mult_3": 0.965,
        "reserve_stress_vol": 2.35,
        "sentinel_calm_mult": 1.1,
        "inv_tight_12": 0.84,
        "inv_tight_3": 0.935,
    }
    ...
    def _depth_levels(self, p) -> int:
        if self.shock_remaining > 0:
            return 1
        v = self.vol_estimate
        if v >= p["depth_vol_one"]:
            return 1
        if v >= p["depth_vol_two"]:
            return 2
        if v >= p["depth_vol_three"]:
            return 3
        d = 4
        if self.toxic_cooldown > 0:
            d = min(d, int(p["toxic_max_levels"]))
        return d
    ...
    def on_step(self, state: StepState):
        ...
        if arb_like:
            self.toxic_cooldown = int(p["toxic_cooldown_steps"])
        else:
            self.toxic_cooldown = max(0, self.toxic_cooldown - 1)
        ...
        if n_levels <= 2:
            vol_scale *= p["stress_size_mult_12"]
        elif n_levels == 3:
            vol_scale *= p["stress_size_mult_3"]

        base_size = p["base_size"]
        max_inv_nom = max(p["max_inventory"], 1e-9)
        if n_levels <= 2:
            eff_max_inv = max(max_inv_nom * p["inv_tight_12"], 1e-9)
        elif n_levels == 3:
            eff_max_inv = max(max_inv_nom * p["inv_tight_3"], 1e-9)
        else:
            eff_max_inv = max_inv_nom

        min_size = p["min_size"]
        bid_size = max(min_size, base_size * vol_scale * max(0.0, 1.0 - net_inv / eff_max_inv))
        ask_size = max(min_size, base_size * vol_scale * max(0.0, 1.0 + net_inv / eff_max_inv))
        ...
        if n_levels <= 2:
            reserve_need += p["reserve_stress_vol"] * vol_ref
        spendable = max(0.0, state.free_cash - reserve_need)

        shock_ask_suppress = False
        shock_bid_suppress = False
        if self.shock_remaining > 0:
            if self.shock_sign < 0:
                bid_size = 0.0
                shock_bid_suppress = True
            elif self.shock_sign > 0:
                ask_size = 0.0
                shock_ask_suppress = True
        ...
        level_specs = [
            (int(p["l2_spread_extra"]), p["l2_size_frac"]),
            (int(p["l3_spread_extra"]), p["l3_size_frac"]),
            (int(p["l4_spread_extra"]), p["l4_size_frac"]),
        ]
        for idx, (extra_ticks, frac) in enumerate(level_specs, start=2):
            if n_levels < idx:
                break
            if shock_bid_suppress:
                bid_sz = 0.0
            else:
                bid_sz = max(min_size, bid_size * frac) if bid_size > 0 else 0.0
            if shock_ask_suppress:
                ask_sz = 0.0
            else:
                ask_sz = max(min_size, ask_size * frac) if ask_size > 0 else 0.0

            bid_L = max(1, my_bid - extra_ticks)
            ask_L = min(99, my_ask + extra_ticks)

            if bid_sz > 0:
                cost_b = bid_L * 0.01 * bid_sz
                if cost_b <= spendable:
                    actions.append(PlaceOrder(side=Side.BUY, price_ticks=bid_L, quantity=bid_sz))
                    spendable -= cost_b

            if ask_sz > 0:
                cov_x = min(ask_sz, avail_yes)
                unc_x = max(0.0, ask_sz - cov_x)
                coll_x = (1.0 - ask_L * 0.01) * unc_x
                if coll_x <= spendable + 1e-9:
        ...
        sentinel_ok = (
            n_levels >= 4
            and comp_spread >= int(p["sentinel_min_spread"])
            and self.quiet_steps >= int(p["sentinel_quiet_steps"])
            and self.shock_remaining <= 0
        )
        if sentinel_ok:
            sent_sz = round(p["sentinel_size"] * p["sentinel_calm_mult"] * vol_scale, 2)
            tick_b = max(1, bid + 1)
            sz_b = round(max(0.1, sent_sz * max(0.0, 1.0 - net_inv / eff_max_inv)), 2)
            ...
            if sz_b > 0.1 and cost_b <= spendable:
                ...
                spendable -= cost_b
            tick_a = min(99, ask - 1)
            sz_a = round(max(0.1, sent_sz * max(0.0, 1.0 + net_inv / eff_max_inv)), 2)
