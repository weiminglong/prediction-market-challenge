"""I4: multilevel quoting with adaptive inside-spread sentinels.

Sentinel size/placement scale with competitor spread width, short-term volatility,
and a smoothed fill-imbalance signal; sentinels are throttled or skipped under
toxicity (arb-like flow, extreme imbalance bursts, high vol).
"""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # Adaptive sentinel
        "sentinel_spread_ref": 6.0,
        "sentinel_spread_bonus": 0.11,
        "sentinel_spread_scale_cap": 1.45,
        "sentinel_spread_per_tick": 3,
        "sentinel_inside_max": 3,
        "sentinel_vol_coeff": 3.15,
        "sentinel_vol_floor": 0.22,
        "sentinel_vol_kill": 1.55,
        "imbalance_decay": 0.82,
        "sentinel_imb_damp": 0.38,
        "sentinel_imb_toxic": 0.78,
        "sentinel_calm_vol_max": 0.95,
    }
    ...
    def on_step(self, state: StepState):
        ...
        step_imb = 0.0
        if total_fills > 1e-9:
            step_imb = (buy_qty - sell_qty) / total_fills
        id_ = p["imbalance_decay"]
        self.fill_imbalance_ema = id_ * self.fill_imbalance_ema + (1.0 - id_) * step_imb
        ...
        comp_spread = ask - bid
        min_sp = int(p["sentinel_min_spread"])
        quiet_need = int(p["sentinel_quiet_steps"])
        imb_tox = total_fills > 1.5 and abs(step_imb) > p["sentinel_imb_toxic"]
        vol_tox = self.vol_estimate > p["sentinel_vol_kill"]
        calm_ok = self.vol_estimate <= p["sentinel_calm_vol_max"]

        place_sentinel = (
            comp_spread >= min_sp
            and self.quiet_steps >= quiet_need
        ...
        if place_sentinel:
            spread_excess = max(0, comp_spread - min_sp)
            sref = max(p["sentinel_spread_ref"], 1e-6)
            spread_scale = 1.0 + p["sentinel_spread_bonus"] * (spread_excess / sref)
            spread_scale = min(spread_scale, p["sentinel_spread_scale_cap"])

            vol_damp_sent = max(
                p["sentinel_vol_floor"],
                1.0 - self.vol_estimate * p["sentinel_vol_coeff"],
            )

            base_sent = p["sentinel_size"] * vol_scale * spread_scale * vol_damp_sent
            imb_d = p["sentinel_imb_damp"]
            bid_imb_mult = max(0.35, 1.0 - imb_d * max(0.0, self.fill_imbalance_ema))
            ask_imb_mult = max(0.35, 1.0 - imb_d * max(0.0, -self.fill_imbalance_ema))

            max_inside = min(int(p["sentinel_inside_max"]), max(1, (comp_spread - 1) // 2))
            extra_inside = spread_excess // max(int(p["sentinel_spread_per_tick"]), 1)
            inside_span = max(1, min(max_inside, 1 + extra_inside))

            tick_b = max(1, bid + inside_span)
            tick_a = min(99, ask - inside_span)
            if tick_b >= tick_a:
                tick_b = max(1, bid + 1)
                tick_a = min(99, ask - 1)

            sz_b = round(
                max(0.1, base_sent * max(0.0, 1.0 - net_inv / max_inv) * bid_imb_mult),
                2,
            )
            sz_a = round(
                max(0.1, base_sent * max(0.0, 1.0 + net_inv / max_inv) * ask_imb_mult),
                2,
            )
