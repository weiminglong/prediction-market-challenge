"""K4: asymmetric trend-leaning maker with fast reversal de-risk (from strat2_team combined)."""
...
class Strategy(BaseStrategy):
    ...
    def _trend_clip(self) -> float:
        return _clamp(self.trend / 2.35, -1.0, 1.0)
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            rev_k = max(1.12, 1.62 * max(self.vol_ema, 0.27))
            against_long = net_inv > 1.35 and move <= -rev_k
            against_short = net_inv < -1.35 and move >= rev_k
            trend_flip = old_trend * self.trend < 0 and abs(old_trend) > 0.09 and abs(self.trend) > 0.07
            counter_trend = old_trend * move < 0 and abs(move) >= rev_k and abs(old_trend) > 0.11
            if against_long or against_short or trend_flip or counter_trend:
                self.reversal_remaining = max(self.reversal_remaining, 3)
        ...
        tc = self._trend_clip()
        fair = mid + self.fill_bias + self._convex_inv_shift(net_inv) + self.trend * 1.02
        ...
        asym_ticks = int(round(1.05 * abs(tc)))
        if tc > 0:
            bid_half = max(1, base_half - asym_ticks)
            ask_half = max(1, base_half + asym_ticks)
        elif tc < 0:
            bid_half = max(1, base_half + asym_ticks)
            ask_half = max(1, base_half - asym_ticks)
        else:
            bid_half = ask_half = base_half
        ...
        vol_scale = max(0.06, 1.0 - self.vol_ema * 2.38)
        if self.shock_remaining > 0:
            vol_scale *= 0.15
        if self.reversal_remaining > 0:
            vol_scale *= 0.52
        ...
        sz_asym = 0.20 * tc
        bid_size *= max(0.72, 1.0 + sz_asym)
        ask_size *= max(0.72, 1.0 - sz_asym)
        ...
        if self.reversal_remaining > 0:
            if net_inv > 0.75:
                bid_size *= 0.14
                bid_half_eff = bid_half + 2
                my_bid = max(1, int(round(fair - bid_half_eff)))
            elif net_inv < -0.75:
                ask_size *= 0.14
                ask_half_eff = ask_half + 2
                my_ask = min(99, int(round(fair + ask_half_eff)))
            if my_bid >= my_ask:
                return actions
