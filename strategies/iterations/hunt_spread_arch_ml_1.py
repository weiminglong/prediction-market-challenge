"""Arch base + multi-level: ±1 (size 1) + ±2 (size 10)."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Inner level: ±1, size 1.0
        inner_bid = max(1, int(round(fair - 1)))
        inner_ask = min(99, int(round(fair + 1)))

        if inner_bid < inner_ask:
            inner_bid_size = round(1.0 * vol_scale * bid_inv_scale, 1)
            inner_ask_size = round(1.0 * vol_scale * ask_inv_scale, 1)
        ...
        if my_bid < my_ask:
            bid_size = max(0.2, 10.0 * vol_scale * bid_inv_scale)
            ask_size = max(0.2, 10.0 * vol_scale * ask_inv_scale)
