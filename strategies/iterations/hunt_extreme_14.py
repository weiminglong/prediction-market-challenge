"""Extreme variant 14: Zero cooldown quoting + dynamic inv spread + size 10, inv_cap 10.
Combines V12's withdrawal after fill with V11's dynamic spread widening on risky side."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Dynamic spread: widen on extending side
        abs_inv = abs(net_inv)
        inv_widen = min(2, int(abs_inv / 4))

        if net_inv > 0:
            bid_spread = 2 + vol_widen + inv_widen
            ask_spread = 2 + vol_widen
        elif net_inv < 0:
            bid_spread = 2 + vol_widen
            ask_spread = 2 + vol_widen + inv_widen
        else:
            bid_spread = 2 + vol_widen
            ask_spread = 2 + vol_widen

        # On cooldown: completely withdraw
