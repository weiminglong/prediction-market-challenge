"""V79: More stable - focus on consistent retail capture with moderate risk."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            move = mid - prev_mid
            # Slower momentum decay for stability
            self.comp_momentum = 0.4 * self.comp_momentum + 0.6 * move
            # Slower vol estimation
        ...
        # Conservative vol scaling for stability
        vol_scale = max(0.25, 1.0 - self.vol_estimate * 0.4)
        base_size = 7.0 * vol_scale
        ...
        if cost <= state.free_cash:
            actions.append(
                PlaceOrder(side=Side.BUY, price_ticks=my_bid, quantity=bid_size)
            )

        actions.append(
            PlaceOrder(side=Side.SELL, price_ticks=my_ask, quantity=ask_size)
        )
