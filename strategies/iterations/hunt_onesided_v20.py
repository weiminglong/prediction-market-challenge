"""One-sided quoting v20: v15 with inv_skew=0.08 and safe-side spread=2 during cooldown.

Use tighter spread on safe side during cooldown to get more retail fills.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # During cooldown: safe side gets normal spread (2), not wide (3)
        if self.bid_cooldown > 0:
            bid_spread = 3 + vol_widen  # doesn't matter, size is 0
            ask_spread = 2 + vol_widen  # tight on safe side
        elif self.ask_cooldown > 0:
            bid_spread = 2 + vol_widen  # tight on safe side
            ask_spread = 3 + vol_widen  # doesn't matter, size is 0
        else:
        ...
        if self.bid_cooldown > 0:
            bid_size = 0.0
            ask_size = 15.0 * vol_scale * max(0.0, 1.0 + net_inv / max_inv)
        elif self.ask_cooldown > 0:
            bid_size = 15.0 * vol_scale * max(0.0, 1.0 - net_inv / max_inv)
            ask_size = 0.0
        else:
