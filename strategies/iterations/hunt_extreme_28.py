"""Extreme variant 28: Place only the safe-side order at spread 2, and the risky-side at spread 3.
Combined with zero cooldown quoting. This is a hybrid between V12 and V11."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Asymmetric spread: tight on safe side, wider on risky side
        if net_inv > 0:
            # Long YES: ask is safe (spread 2), bid is risky (spread 3)
            bid_spread = 3 + vol_widen
            ask_spread = 2 + vol_widen
        elif net_inv < 0:
            # Long NO: bid is safe (spread 2), ask is risky (spread 3)
