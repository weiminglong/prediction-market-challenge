"""One-sided quoting v7: Hybrid - base is strategy_final but with asymmetric kill.

Start from strategy_final's proven approach, but when inventory exceeds threshold,
completely kill the accumulating side (set size to 0) instead of just reducing it.
This is the minimal change from baseline to test pure one-sided concept.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # ONE-SIDED KILL: when inventory exceeds threshold, zero out accumulating side
        kill_threshold = 3
        max_inv = 10

        if net_inv > kill_threshold:
            # Long: kill bids entirely, boost asks
            bid_size = 0.0
            ask_size = max(0.2, ask_base * vol_scale * min(2.0, 1.0 + net_inv / max_inv))
        elif net_inv < -kill_threshold:
            # Short: kill asks entirely, boost bids
            bid_size = max(0.2, bid_base * vol_scale * min(2.0, 1.0 - net_inv / max_inv))
            ask_size = 0.0
        else:
            # Normal two-sided with inventory scaling
