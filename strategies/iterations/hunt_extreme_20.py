"""Extreme variant 20: Asymmetric cooldown approach.
After buy fill: withdraw bid but keep ask (with wide spread).
After sell fill: withdraw ask but keep bid (with wide spread).
This ensures the safe side always captures retail while avoiding arb on the dangerous side."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if state.buy_filled_quantity > 0:
            self.fill_bias -= 1.0
            self.bid_cooldown = 2  # withdraw bid
        if state.sell_filled_quantity > 0:
            self.fill_bias += 1.0
            self.ask_cooldown = 2  # withdraw ask
        self.fill_bias *= 0.5
        ...
        # On bid cooldown: withdraw bid completely, keep ask at spread 2
        # On ask cooldown: withdraw ask completely, keep bid at spread 2
        ...
        # Safe side gets large size always
