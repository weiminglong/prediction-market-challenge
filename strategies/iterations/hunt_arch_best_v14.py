"""Spread=3 base (wider), no cd, no mom, fill_bias decay=0.4, vol thresh=1.5, coeff=0.9."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        vol_widen = 1 if self.vol_estimate > 1.5 else 0
        my_bid = max(1, int(round(fair - 3 - vol_widen)))
        my_ask = min(99, int(round(fair + 3 + vol_widen)))

        if my_bid >= my_ask:
            return actions

        vol_scale = max(0.2, 1.0 - self.vol_estimate * 0.9)
        base_size = 10.0
        max_inv = 10
