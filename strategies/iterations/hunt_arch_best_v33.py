"""Combo: zero filled side + vol_coeff=1.5 + fill_bias. After fill, skip filled side entirely."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Track which side to skip (from previous step fills)
        prev_skip_bid = self.skip_bid
        prev_skip_ask = self.skip_ask
        self.skip_bid = state.buy_filled_quantity > 0
        self.skip_ask = state.sell_filled_quantity > 0
        ...
        # Skip the side that was just filled (arb likely hit it)
        if not self.skip_bid:
            cost = my_bid * 0.01 * bid_size
            if cost <= state.free_cash:
                actions.append(PlaceOrder(side=Side.BUY, price_ticks=my_bid, quantity=bid_size))

        if not self.skip_ask:
