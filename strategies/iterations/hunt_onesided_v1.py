"""One-sided quoting v1: threshold=3, reducing_size=30, accum_size=0, flat_size=15.

When inventory is flat (|inv| <= 3), quote both sides with large size.
When long YES (inv > 3), ONLY quote asks (sell) with massive size to unwind.
When short (inv < -3), ONLY quote bids (buy) with massive size to unwind.
This eliminates arb adverse selection on the accumulating side entirely.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # One-sided quoting parameters
        threshold = 3
        flat_size = 15.0
        reducing_size = 30.0
        accum_size = 0.0  # zero on accumulating side
        ...
        if net_inv > threshold:
            # Long YES: only sell (ask) to reduce
            ask_spread = 1 + vol_widen  # tight spread for faster unwinding
            ...
            # Tiny or zero bid
            if accum_size > 0:
                bid_spread = 3 + vol_widen
                my_bid = max(1, int(round(fair - bid_spread)))
                if my_bid < my_ask:
                    cost = my_bid * 0.01 * accum_size
                    if cost <= state.free_cash:
                        actions.append(PlaceOrder(side=Side.BUY, price_ticks=my_bid, quantity=accum_size))

        elif net_inv < -threshold:
            # Short (long NO): only buy (bid) to reduce
            bid_spread = 1 + vol_widen  # tight spread for faster unwinding
            my_bid = max(1, int(round(fair - bid_spread)))
            ...
            if accum_size > 0:
                ...
                my_ask = min(99, int(round(fair + ask_spread)))
                if my_ask > my_bid:
                    actions.append(PlaceOrder(side=Side.SELL, price_ticks=my_ask, quantity=accum_size))

        else:
            # Flat: quote both sides with good size
