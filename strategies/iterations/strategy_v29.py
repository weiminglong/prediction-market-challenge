"""V29: V26 + vol-adaptive spread (V25 idea) combined.

Widen spread by 1 tick during high vol, AND scale down size.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Cooldown-based spread
        bid_base = 3 if self.bid_cooldown > 0 else 2
        ask_base = 3 if self.ask_cooldown > 0 else 2

        # Vol-adaptive spread: extra tick when vol > 1.5
        vol_extra = 1 if self.vol_estimate > 1.5 else 0
