"""Extreme variant 21: Use competitor spread width as a signal.
Wide competitor spread = more uncertainty = wider our spread.
Narrow competitor spread = confident = tighter spread possible.
Also: stronger fill_bias shift (1.2) + zero cooldown quoting."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Use competitor spread as signal
        vol_widen = 1 if self.vol_estimate > 1.0 else 0
        spread_widen = 1 if comp_spread > 6 else 0  # wide competitor spread = danger

        bid_spread = 2 + vol_widen + spread_widen
        ask_spread = 2 + vol_widen + spread_widen
