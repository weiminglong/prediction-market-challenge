"""Extreme variant 3: Size 30, inv_cap 3, step function (full or zero), inv_skew 0.25."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        inv_skew = -net_inv * 0.25  # very aggressive skew
        ...
        bid_base = 0.8 if self.bid_cooldown > 0 else 30.0
        ask_base = 0.8 if self.ask_cooldown > 0 else 30.0

        # Step function: full size if |inv| < 3, zero otherwise
        max_inv = 3
        bid_size = bid_base * vol_scale if net_inv < max_inv else 0.0
        ask_size = ask_base * vol_scale if net_inv > -max_inv else 0.0
