"""One-sided quoting v15: Zero during cooldown + extra large opposite side.

When bid fills (arb), zero bid for 2 steps but make ask size 15 (not 10).
This focuses on maximizing retail capture on the safe side after arb detection.
Baseline approach otherwise.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Cooldown: zero on filled side, boost opposite
        if self.bid_cooldown > 0:
            bid_size = 0.0  # zero after bid fill
            ...
            ask_size = 0.0  # zero after ask fill
        else:
