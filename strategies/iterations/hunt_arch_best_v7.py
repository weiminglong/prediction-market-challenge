"""Best arch: no cd, no mom. Try fill_bias only on arb (quantity-weighted)."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Quantity-weighted fill bias
        if state.buy_filled_quantity > 0:
            self.fill_bias -= state.buy_filled_quantity * 0.2
        if state.sell_filled_quantity > 0:
            self.fill_bias += state.sell_filled_quantity * 0.2
        self.fill_bias *= 0.5
