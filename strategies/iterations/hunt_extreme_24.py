"""Extreme variant 24: Pure safe-side market making.
ONLY quote on the inventory-reducing side. Never extend inventory.
When flat, quote both sides at spread 2 with large size.
This should have minimal arb losses since we only offer liquidity on the safe side."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Only quote safe side (or both when flat)
        if net_inv > 1:
            # Long YES: only sell (ask)
            ask_size = max(0.2, 10.0 * vol_scale)
            actions.append(PlaceOrder(side=Side.SELL, price_ticks=my_ask, quantity=ask_size))
        elif net_inv < -1:
            # Long NO: only buy (bid)
            bid_size = max(0.2, 10.0 * vol_scale)
