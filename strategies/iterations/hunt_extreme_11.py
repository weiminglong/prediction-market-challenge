"""Extreme variant 11: Dynamic spread based on inventory. Tight spread (2) when inv is balanced,
wide spread (4) when extended. This should cut arb losses when vulnerable while
capturing retail when safe. Size 10, inv_cap 10 (like baseline)."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Dynamic spread: wider on the extending side based on inventory
        abs_inv = abs(net_inv)
        inv_widen = min(2, int(abs_inv / 3))  # 0 for inv<3, 1 for 3-5, 2 for 6+

        if net_inv > 0:
            # Long YES: widen bid (risky), keep ask tight (safe)
            bid_spread = (3 if self.bid_cooldown > 0 else 2) + vol_widen + inv_widen
            ask_spread = (3 if self.ask_cooldown > 0 else 2) + vol_widen
        elif net_inv < 0:
            # Long NO: widen ask (risky), keep bid tight (safe)
            bid_spread = (3 if self.bid_cooldown > 0 else 2) + vol_widen
            ask_spread = (3 if self.ask_cooldown > 0 else 2) + vol_widen + inv_widen
        else:
            bid_spread = (3 if self.bid_cooldown > 0 else 2) + vol_widen
            ask_spread = (3 if self.ask_cooldown > 0 else 2) + vol_widen
