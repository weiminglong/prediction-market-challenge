"""Extreme variant 2: Size 25, inv_cap 4, sqrt scaling, inv_skew 0.20."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        inv_skew = -net_inv * 0.20  # extreme skew
        ...
        bid_base = 0.8 if self.bid_cooldown > 0 else 25.0
        ask_base = 0.8 if self.ask_cooldown > 0 else 25.0

        # Sqrt scaling: fast cutoff near cap
        max_inv = 4
        if net_inv > 0:
            bid_ratio = max(0.0, 1.0 - (abs(net_inv) / max_inv) ** 0.5)
        else:
            bid_ratio = 1.0
        if net_inv < 0:
            ask_ratio = max(0.0, 1.0 - (abs(net_inv) / max_inv) ** 0.5)
        else:
            ask_ratio = 1.0
