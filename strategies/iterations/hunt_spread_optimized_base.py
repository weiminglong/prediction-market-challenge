"""Optimized baseline: ±2 spread with tuned parameters.

Observations from experiments:
- ±1 spread generates proportional arb, net negative
- Multi-level adds retail but also arb, roughly neutral
- Size 12 outer at ±2 gives retail 18 but arb -13.6

Key tuning: the original uses vol_scale with 0.7 multiplier. This may be
too aggressive. Also, the cooldown size 0.8 might be too small (missing
retail on the safe side during cooldown).

Try: cooldown size 2.0, inv_cap 8 (not 10), inv_skew 0.08 (stronger).
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        inv_skew = -net_inv * 0.08  # Slightly stronger than 0.06
        ...
        # Larger cooldown size to keep capturing retail on safe side
        bid_base = 2.0 if self.bid_cooldown > 0 else 10.0
        ask_base = 2.0 if self.ask_cooldown > 0 else 10.0
