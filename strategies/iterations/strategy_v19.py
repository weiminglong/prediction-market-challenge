"""V19: V17 + tighter non-hit side (±1 instead of ±2).

After a fill, widen the hit side to ±3, tighten the other side to ±1
to capture more retail on the side where arb just created flow.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Asymmetric: widen hit side to 3, tighten non-hit to 1
        if self.last_buy_fill or self.last_sell_fill:
            bid_spread = 3 if self.last_buy_fill else 1
            ask_spread = 3 if self.last_sell_fill else 1
        else:
            bid_spread = 2
            ask_spread = 2
