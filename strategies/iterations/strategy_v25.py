"""V25: V23 + volatility-adaptive spread AND size.

During high vol: widen spread AND reduce size (double protection).
During calm: tighter spread, full size (maximize retail).
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Base spread from cooldown
        bid_base = 3 if self.bid_cooldown > 0 else 2
        ask_base = 3 if self.ask_cooldown > 0 else 2

        # Volatility-adaptive spread: add 1 tick when volatile
        vol_extra = 1 if self.vol_estimate > 1.0 else 0
        bid_spread = bid_base + vol_extra
        ask_spread = ask_base + vol_extra
