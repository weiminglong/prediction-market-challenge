"""Extreme variant 8: One-sided quoting bias. Only quote heavily on side that reduces inventory.
Small orders on extending side, large on reducing side. Goal: maximize retail flow on safe side."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Extreme asymmetry: huge on inventory-reducing side, tiny on extending side
        if net_inv > 0:
            # Long YES, want to sell more (ask is safe, bid is risky)
            bid_base = 0.5 if self.bid_cooldown > 0 else 2.0  # small extending
            ask_base = 0.5 if self.ask_cooldown > 0 else 15.0  # large reducing
        elif net_inv < 0:
            # Long NO, want to buy more (bid is safe, ask is risky)
            bid_base = 0.5 if self.bid_cooldown > 0 else 15.0  # large reducing
            ask_base = 0.5 if self.ask_cooldown > 0 else 2.0  # small extending
        else:
