"""v32: cooldown=1 zero + no vol_estimate scaling (remove vol_scale entirely).

The vol_scale might be reducing our size too much. Try without it.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self.bid_cooldown > 0:
            bid_size = 0.0
            ask_size = 10.0 * max(0.0, 1.0 + net_inv / max_inv)
        elif self.ask_cooldown > 0:
            bid_size = 10.0 * max(0.0, 1.0 - net_inv / max_inv)
            ask_size = 0.0
        else:
            bid_size = max(0.2, 10.0 * max(0.0, 1.0 - net_inv / max_inv))
            ask_size = max(0.2, 10.0 * max(0.0, 1.0 + net_inv / max_inv))
