"""Extreme variant 18: Detect arb by looking at competitor mid moves.
When mid moves sharply, widen spread or withdraw. Use EMA of absolute moves.
When calm, quote aggressively tight (spread 1). When jumpy, spread 3+."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self.recent_move >= 3:
            # Very jumpy - withdraw completely
            return actions
        elif self.vol_estimate > 1.5:
            base_spread = 3
        elif self.vol_estimate > 0.5:
            base_spread = 2
        else:
            base_spread = 1  # very tight when calm

        bid_spread = (base_spread + 1 if self.bid_cooldown > 0 else base_spread)
        ask_spread = (base_spread + 1 if self.ask_cooldown > 0 else base_spread)
