"""One-sided quoting v18: v15 with larger boost (20) on safe side during cooldown.

v15 got +6.40. Try boosting the safe side to 20 during cooldown.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Cooldown: zero on filled side, boost opposite to 20
        if self.bid_cooldown > 0:
            bid_size = 0.0
            ask_size = 20.0 * vol_scale * max(0.0, 1.0 + net_inv / max_inv)
        elif self.ask_cooldown > 0:
            bid_size = 20.0 * vol_scale * max(0.0, 1.0 - net_inv / max_inv)
            ask_size = 0.0
        else:
