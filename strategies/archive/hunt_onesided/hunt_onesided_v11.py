"""One-sided quoting v11: Combined cooldown zero + inventory one-sided.

Combine v7 (inventory kill) and v8 (cooldown zero) approaches.
During cooldown: zero on filled side.
When inv > threshold: zero on accum side.
Both: maximum protection from arb.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Cooldown override: zero on filled side
        if self.bid_cooldown > 0:
            bid_size = 0.0
        if self.ask_cooldown > 0:
            ask_size = 0.0

        # Inventory kill: zero on accum side when beyond threshold
        kill_threshold = 3
