"""v27: cooldown=1, safe-side boost=15, normal size=12.
Boost baseline size from 10 to 12, safe-side cooldown to 15."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self.bid_cooldown > 0:
            ...
            bid_size = max(0.2, 12.0 * vol_scale * max(0.0, 1.0 - net_inv / max_inv))
            ask_size = max(0.2, 12.0 * vol_scale * max(0.0, 1.0 + net_inv / max_inv))
