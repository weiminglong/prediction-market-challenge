"""No-cooldown v6: v5 winner but with size 12 outer and inner size 1.0.

V5 hit 6.46 with size=10 outer + 0.5 inner at ±1.
V3 showed size=12 generates huge retail (24.0).
Combine: size 12 outer + inner 1.0.
"""
    ...
    def on_step(self, state: StepState):
        ...
        if self.vol_estimate < 0.5:
            ...
            if inner_bid < inner_ask:
                cost = inner_bid * 0.01 * 1.0
                if cost <= cash_remaining:
                    actions.append(PlaceOrder(side=Side.BUY, price_ticks=inner_bid, quantity=1.0))
                    cash_remaining -= cost
                actions.append(PlaceOrder(side=Side.SELL, price_ticks=inner_ask, quantity=1.0))
        ...
        if my_bid < my_ask:
            bid_size = max(0.5, round(12.0 * vol_scale * bid_inv_scale, 1))
            ask_size = max(0.5, round(12.0 * vol_scale * ask_inv_scale, 1))
