"""Hybrid best: combine all winning insights.

Winning insights so far:
1. ±2 base spread is optimal (±1 feeds arb proportionally)
2. Size 10-12 at ±2 captures retail well
3. Tight inv cap forces rapid turnover
4. Inner ±1 only adds marginal value
5. Cooldown of 2 at ±3 is right balance

New idea: instead of multi-level, use ±2 with ADAPTIVE size.
When inventory is building, reduce size. When flat, maximize size.
Also: use competitor spread width as signal (wide = calm = more aggressive).
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Adaptive size: bigger when competitor spread is wide (calm market)
        spread_bonus = max(0.0, (comp_spread - 4) * 0.5)  # bonus when spread > 4
        base_size = 10.0 + spread_bonus
