"""Tight ±1 spread with large size and tight inventory cap.

Hypothesis: ±1 spread captures much more retail flow. Larger size compensates
for increased arb losses. Tight inv cap (6) forces rapid turnover.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Tight ±1 base spread, ±2 on cooldown
        bid_spread = 2 if self.bid_cooldown > 0 else 1
        ask_spread = 2 if self.ask_cooldown > 0 else 1
        ...
        # Large size to capture more retail
        vol_scale = max(0.2, 1.0 - self.vol_estimate * 0.7)

        bid_base = 1.0 if self.bid_cooldown > 0 else 20.0
        ask_base = 1.0 if self.ask_cooldown > 0 else 20.0
