"""Wide and safe: ±3 base spread with massive size.

Contrarian hypothesis: wider spread = less arb, but at ±3 with size 20+,
we capture a LOT of retail (any order that sweeps past the competitor
hits us with big size). The arb might not bother at ±3.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        bid_spread = 4 if self.bid_cooldown > 0 else 3
        ask_spread = 4 if self.ask_cooldown > 0 else 3
        ...
        bid_base = 1.0 if self.bid_cooldown > 0 else 25.0
        ask_base = 1.0 if self.ask_cooldown > 0 else 25.0
