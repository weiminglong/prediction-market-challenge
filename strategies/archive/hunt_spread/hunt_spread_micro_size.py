"""Micro size at ±2: tiny orders (size 3) that are hard for arb to profit from.

Hypothesis: the arb sweeps our large orders. If we quote tiny (size 3),
each arb fill costs them relatively more in execution overhead while we
still accumulate retail fills over time. Lower variance, lower edge per
trade, but better ratio.
"""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        # Micro size: 3.0 base
        bid_base = 0.5 if self.bid_cooldown > 0 else 3.0
        ask_base = 0.5 if self.ask_cooldown > 0 else 3.0
