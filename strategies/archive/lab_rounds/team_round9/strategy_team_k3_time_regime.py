"""K3: combined-style MM with explicit early / mid / late (time-to-end) regimes.

Spread, size, and effective inventory tolerance scale with fraction of horizon left.
Late phase adds flattening bias and hard endgame guards (fill-bias + vol + shock)
to avoid blowups — distinct from round8 regime hysteresis and bid/ask tox EWMA.
"""
...
class Strategy(BaseStrategy):
    # Normalized time left: steps_remaining / (step + steps_remaining)
    _FRAC_EARLY = 0.63
    _FRAC_LATE = 0.17

    # Endgame anti-tox (steps_remaining absolute, works with default n_steps=2000)
    _ENDGAME_SOFT = 95
    _ENDGAME_HARD = 42
    _FILL_BIAS_TOX = 1.32
    _VOL_TOX = 1.18
    ...
    @staticmethod
    def _frac_time_left(state: StepState) -> float:
        h = max(1, state.step + state.steps_remaining)
        return state.steps_remaining / float(h)

    def _time_regime(self, frac_left: float) -> str:
        if frac_left > self._FRAC_EARLY:
            return "early"
        if frac_left < self._FRAC_LATE:
            return "late"
        return "mid"
    ...
    def on_step(self, state: StepState):
        ...
        frac_left = self._frac_time_left(state)
        regime = self._time_regime(frac_left)
        ...
        if regime == "early":
            if self.vol_ema < 1.05:
                half_spread += 1
            max_inv *= 1.09
            base_size_mult *= 1.048
            vol_scale_mult *= 1.032
        elif regime == "late":
            max_inv *= 0.92
            base_size_mult *= 0.965
            if abs(net_inv) > 3.8:
                half_spread += 1
            inv_mag = abs(net_inv)
            if inv_mag > 1.85:
                flatten = min(1.0, (inv_mag - 1.85) / 6.2)
                if net_inv > 0:
                    bid_size_pen = 1.0 - 0.36 * flatten
                    ask_size_pen = 1.0 + 0.11 * flatten
                else:
                    bid_size_pen = 1.0 + 0.11 * flatten
                    ask_size_pen = 1.0 - 0.36 * flatten
            else:
                bid_size_pen = ask_size_pen = 1.0
        else:
            bid_size_pen = ask_size_pen = 1.0

        endgame_stress = (
            steps_rem <= self._ENDGAME_SOFT
            and (
                abs(self.fill_bias) >= self._FILL_BIAS_TOX
                or self.vol_ema >= self._VOL_TOX
                or self.shock_remaining > 0
            )
        )
        if endgame_stress:
            half_spread += 1
            base_size_mult *= 0.87
            if steps_rem <= self._ENDGAME_HARD:
                base_size_mult *= 0.91
                max_inv *= 0.96
        ...
        base_size = 10.0 * model_boost * base_size_mult
        vol_scale = max(0.06, 1.0 - self.vol_ema * 2.4) * vol_scale_mult
        if self.shock_remaining > 0:
            vol_scale *= 0.15
        ...
        if regime == "late":
            bid_size *= bid_size_pen
            ask_size *= ask_size_pen
        ...
        if steps_rem <= self._ENDGAME_HARD and abs(net_inv) >= 6.35:
            if net_inv > 0:
                bid_size = 0.0
            else:
                ask_size = 0.0
