"""Q2 holdout guard core + tiny calm-only aligned L2/L3 depth boost (strict low-vol cap)."""
...
class Strategy(BaseStrategy):
    ...
    # Calm-only depth: very low realized vol, quiet regime/tox; per-side cap tied to skew alignment.
    _CALM_VOL_MAX = 0.175
    _CALM_REGIME_MAX = 0.285
    _CALM_TOX_ABS_MAX = 0.42
    _CALM_BOOST_GAIN = 0.048
    _CALM_BOOST_CAP = 1.048
    ...
    def _calm_aligned_l23_mult(self, net_inv: float, regime_raw: float) -> tuple[float, float]:
        if self.vol_ema >= self._CALM_VOL_MAX:
            return 1.0, 1.0
        if regime_raw >= self._CALM_REGIME_MAX:
            return 1.0, 1.0
        if abs(self.fill_bias) >= self._CALM_TOX_ABS_MAX:
            return 1.0, 1.0
        if self.vol_burst_rem > 0:
            return 1.0, 1.0
        ...
        g = self._CALM_BOOST_GAIN
        bid_m = min(self._CALM_BOOST_CAP, 1.0 + g * bid_align)
        ask_m = min(self._CALM_BOOST_CAP, 1.0 + g * ask_align)
        return bid_m, ask_m
    ...
    def on_step(self, state: StepState):
        ...
        calm_b, calm_a = self._calm_aligned_l23_mult(net_inv, regime_raw)
        if vol_burst_active:
        ...
        bid_sz2 = base_b2 * l23_b_scale * calm_b
        ask_sz2 = base_a2 * l23_a_scale * calm_a
        if bid_sz2 < ms:
            bid_sz2 = 0.0
        if ask_sz2 < ms:
            ask_sz2 = 0.0
        ...
        bid_sz3 = base_b3 * l23_b_scale * calm_b
        ask_sz3 = base_a3 * l23_a_scale * calm_a
        if bid_sz3 < ms:
            bid_sz3 = 0.0
        if ask_sz3 < ms:
            ask_sz3 = 0.0
