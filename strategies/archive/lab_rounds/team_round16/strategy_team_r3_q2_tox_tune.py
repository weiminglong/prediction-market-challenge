"""Q2 core (p1 holdout guard) + tox-depth hysteresis and slightly faster fill_bias decay for smoother L2/L3 gating."""
...
class Strategy(BaseStrategy):
    ...
    # Hysteresis vs single 0.90 cutoff: block depth a touch later on the way up, release after a bit more decay.
    _DEPTH_TOX_ENTER = 0.915
    _DEPTH_TOX_EXIT = 0.825
    ...
    def __init__(self):
        ...
        self._tox_depth_block = False

    def _tox_depth_gate_active(self) -> bool:
        a = abs(self.fill_bias)
        if not self._tox_depth_block:
            if a >= self._DEPTH_TOX_ENTER:
                self._tox_depth_block = True
        else:
            if a <= self._DEPTH_TOX_EXIT:
                self._tox_depth_block = False
        return self._tox_depth_block
    ...
    def _depth_allowed(self, risk_off: bool, shock_active: bool, regime_raw: float) -> bool:
        ...
        if self.vol_ema >= self._depth_vol_cap():
            return False
        if self._tox_depth_gate_active():
            return False
    ...
    def on_step(self, state: StepState):
        ...
        bid_sz2 = base_b2 * l23_b_scale
        ask_sz2 = base_a2 * l23_a_scale
        if bid_sz2 < ms:
            bid_sz2 = 0.0
        if ask_sz2 < ms:
            ask_sz2 = 0.0
        ...
        bid_sz3 = base_b3 * l23_b_scale
        ask_sz3 = base_a3 * l23_a_scale
        if bid_sz3 < ms:
            bid_sz3 = 0.0
        if ask_sz3 < ms:
            ask_sz3 = 0.0
