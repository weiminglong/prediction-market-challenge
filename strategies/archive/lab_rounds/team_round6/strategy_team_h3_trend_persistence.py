"""Round 6 H3: G1 regime memory + dual-horizon trend, persistence-based skew, fast reversal pullback."""
...
def _sgn(x: float, eps: float) -> int:
    if x > eps:
        return 1
    if x < -eps:
        return -1
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        # Dual-horizon trend
        "trend_fast_alpha": 0.31,
        "trend_fast_decay": 0.58,
        "trend_slow_alpha": 0.095,
        "trend_slow_decay": 0.805,
        "trend_sign_eps": 0.018,
        # Persistence (EMA of agreement); disagree decays faster
        "persist_build_decay": 0.934,
        "persist_decay_on_disagree": 0.78,
        "persist_gate": 0.40,
        "persist_fair_weight_fast": 0.36,
        "persist_size_boost": 0.14,
        "persist_spread_relief": 1,
        # Reversal: sign clash or fast flip — pullback only for reversal_duration steps
        "reversal_spread_extra": 1,
        "reversal_size_mult": 0.74,
        "reversal_fair_mult": 0.28,
        "reversal_duration": 2,
        "reversal_flip_eps": 0.045,
        "disagree_fair_mult": 0.62,
    }
    ...
    def __init__(self):
        ...
        self.trend_fast = 0.0
        self.trend_slow = 0.0
        self.prev_trend_fast_sgn = 0
        self.persistence = 0.0
        self.shock_remaining = 0
        self.shock_sign = 0
        self.reversal_remaining = 0
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            fa = float(p["trend_fast_alpha"])
            fd = float(p["trend_fast_decay"])
            sa = float(p["trend_slow_alpha"])
            sd = float(p["trend_slow_decay"])
            self.trend_fast = fd * self.trend_fast + fa * move
            self.trend_slow = sd * self.trend_slow + sa * move
        ...
        s_f = _sgn(self.trend_fast, eps)
        s_s = _sgn(self.trend_slow, eps)
        agree = s_f != 0 and s_f == s_s
        disagree = s_f != 0 and s_s != 0 and s_f != s_s

        build_d = float(p["persist_build_decay"])
        disagree_d = float(p["persist_decay_on_disagree"])
        if agree:
            self.persistence = build_d * self.persistence + (1.0 - build_d) * 1.0
        elif disagree:
            self.persistence *= disagree_d
        else:
            self.persistence = build_d * self.persistence + (1.0 - build_d) * 0.35

        flip_eps = float(p["reversal_flip_eps"])
        fast_flipped = (
            self.prev_trend_fast_sgn != 0
            and s_f != 0
            and self.prev_trend_fast_sgn != s_f
            and abs(self.trend_fast) >= flip_eps
        )
        impulse_reversal = disagree or fast_flipped
        if impulse_reversal:
            self.reversal_remaining = max(self.reversal_remaining, int(p["reversal_duration"]))
        self.prev_trend_fast_sgn = s_f

        # G1 regime memory uses slow trend as the structural level
        v_ref = max(self.vol_estimate, float(p["vol_floor"]))
        vol_level = min(1.0, self.vol_estimate / max(float(p["regime_vol_norm"]), 1e-6))
        trend_level = min(1.0, abs(self.trend_slow) / max(float(p["regime_trend_norm"]), 1e-6))
        shift_vol = 0.0
        shift_tr = 0.0
        if self._prev_vol_for_shift is not None and self._prev_trend_for_shift is not None:
            ...
            dt = abs(self.trend_slow - self._prev_trend_for_shift)
        ...
        self._prev_trend_for_shift = self.trend_slow
        ...
        wf = float(p["persist_fair_weight_fast"])
        blend_trend = (1.0 - wf) * self.trend_slow + wf * self.trend_fast
        if self.reversal_remaining > 0:
            blend_trend *= float(p["reversal_fair_mult"])
        elif disagree:
            blend_trend *= float(p["disagree_fair_mult"])
        elif not agree:
            blend_trend *= 0.72
        fair_trend = blend_trend * trend_w
        ...
        if self.reversal_remaining > 0:
            half_spread += int(p["reversal_spread_extra"])
        gate = float(p["persist_gate"])
        relief = int(p["persist_spread_relief"])
        if self.persistence >= gate and agree and self.reversal_remaining == 0:
            half_spread = max(int(p["spread_base"]), half_spread - relief)
        ...
        if self.reversal_remaining > 0:
            vol_scale *= float(p["reversal_size_mult"])
        ...
        boost = float(p["persist_size_boost"])
        eff_p = max(0.0, (self.persistence - gate) / max(1e-6, 1.0 - gate))
        if agree and self.persistence >= gate and self.reversal_remaining == 0 and s_f > 0:
            bid_size *= 1.0 + boost * eff_p
            ask_size *= max(0.72, 1.0 - 0.55 * boost * eff_p)
        elif agree and self.persistence >= gate and self.reversal_remaining == 0 and s_f < 0:
            ask_size *= 1.0 + boost * eff_p
            bid_size *= max(0.72, 1.0 - 0.55 * boost * eff_p)
