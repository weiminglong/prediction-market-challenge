"""Direction J: fade competitor moves (mean-reversion hypothesis)."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        self.momentum = 0.75 * self.momentum + 0.25 * move
        ...
        fair = mid + self.fill_bias - 0.5 * self.momentum - 0.055 * net_inventory

        spread = 2 + (1 if self.vol_estimate > 1.5 else 0)
        my_bid = max(1, int(round(fair - spread)))
        my_ask = min(99, int(round(fair + spread)))
        if my_bid >= my_ask:
            return actions

        vol_scale = max(0.2, 1.0 - self.vol_estimate * 0.42)
