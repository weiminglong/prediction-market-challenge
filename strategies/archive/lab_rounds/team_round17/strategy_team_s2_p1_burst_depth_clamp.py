"""S2 round17: p1 o4 tox×trend core; vol-burst windows clamp L3 hard, L2 partial (L1 unchanged)."""
...
class Strategy(BaseStrategy):
    ...
    _TT_INV_SOFT = 1.15
    _VOL_BURST_STEPS = 4
    _BURST_L2_SCALE = 0.80
    _BURST_L3_SCALE = 0.08
    ...
    def on_step(self, state: StepState):
        ...
        if vol_burst_active:
            bid_sz2 *= self._BURST_L2_SCALE
            ask_sz2 *= self._BURST_L2_SCALE
        if bid_sz2 < ms:
            bid_sz2 = 0.0
        if ask_sz2 < ms:
            ask_sz2 = 0.0
        ...
        if vol_burst_active:
            bid_sz3 *= self._BURST_L3_SCALE
            ask_sz3 *= self._BURST_L3_SCALE
        if bid_sz3 < ms:
            bid_sz3 = 0.0
        if ask_sz3 < ms:
            ask_sz3 = 0.0
