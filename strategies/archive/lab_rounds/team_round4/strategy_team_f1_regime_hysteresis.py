"""Team F1: regime hysteresis (calm / alert / shock / recovery) on tuned trend-shock core."""
...
class Strategy(BaseStrategy):
    PARAMS = {
        ...
        "vol_decay": 0.935142,
        "vol_floor": 0.064398,
        # Slow vol baseline for hysteresis
        "vol_slow_decay": 0.985,
        # Enter / persist ALERT vs CALM (Schmitt-style vs slow vol + move)
        "alert_stress_steps": 2,
        "alert_relax_steps": 4,
        "alert_vol_ratio_hi": 1.2,
        "alert_vol_ratio_lo": 1.06,
        "alert_move_frac_hi": 0.58,
        "alert_move_frac_lo": 0.32,
        "alert_spread_add": 1,
        "alert_size_mult": 0.82,
        "alert_trend_mult": 0.62,
        "alert_pull_mid": 0.22,
        # Shock arming (anti single-tick whipsaw)
        "shock_arm_required": 2,
        "shock_single_bar_mult": 2.35,
        # Recovery: wide, small, fair anchored toward mid
        "recovery_duration": 4,
        "recovery_spread_add": 1,
        "recovery_size_mult": 0.68,
        "recovery_trend_mult": 0.45,
        "recovery_pull_mid": 0.38,
    }
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            sd = float(p["vol_slow_decay"])
            self.vol_slow = sd * self.vol_slow + (1.0 - sd) * abs(move)
        ...
        base_slow = max(self.vol_slow, float(p["shock_vol_floor"]), 1e-6)
        stress_vol = self.vol_estimate >= float(p["alert_vol_ratio_hi"]) * base_slow
        stress_move = abs_move >= float(p["alert_move_frac_hi"]) * shock_trigger
        stress = stress_vol or stress_move
        relaxed_vol = self.vol_estimate <= float(p["alert_vol_ratio_lo"]) * base_slow
        relaxed_move = abs_move <= float(p["alert_move_frac_lo"]) * shock_trigger
        relaxed = relaxed_vol and relaxed_move
        ...
        if abs_move >= shock_trigger and move_sign != 0:
            if move_sign == self.shock_arm_sign:
                ...
                self.shock_arm_steps = 1
        elif abs_move < float(p["alert_move_frac_lo"]) * shock_trigger:
        ...
        if self.regime == Regime.CALM:
            if shock_commit and abs_move >= shock_trigger and move_sign != 0:
                ...
                self.shock_arm_steps = 0
                self.shock_arm_sign = 0
            elif self.stress_streak >= int(p["alert_stress_steps"]):
                self.regime = Regime.ALERT
                self.relax_streak = 0

        elif self.regime == Regime.ALERT:
            if shock_commit and abs_move >= shock_trigger and move_sign != 0:
                self.shock_remaining = int(p["shock_duration"])
                self.shock_sign = move_sign
                self.regime = Regime.SHOCK
                ...
                self.shock_arm_steps = 0
                self.shock_arm_sign = 0
            elif self.relax_streak >= int(p["alert_relax_steps"]):
                self.regime = Regime.CALM
                self.stress_streak = 0
            ...
            if self.recovery_remaining <= 0:
                self.regime = Regime.CALM
                self.stress_streak = 0
            elif shock_commit and abs_move >= shock_trigger and move_sign != 0:
                self.shock_remaining = int(p["shock_duration"])
        ...
        tw = float(p["trend_weight"])
        if self.regime == Regime.ALERT:
            tw *= float(p["alert_trend_mult"])
        elif self.regime == Regime.RECOVERY:
            tw *= float(p["recovery_trend_mult"])

        fair = mid + self.fill_bias - net_inv * float(p["inv_skew"]) + self.trend * tw

        pull = 0.0
        if self.regime == Regime.ALERT:
            pull = float(p["alert_pull_mid"])
        elif self.regime == Regime.RECOVERY:
            pull = float(p["recovery_pull_mid"])
        if pull > 0.0:
            fair = mid + (1.0 - pull) * (fair - mid)

        half_spread = int(p["spread_base"])
        if self.vol_estimate > float(p["spread_vol_threshold"]):
            half_spread += int(p["spread_vol_extra"])
        if self.regime == Regime.ALERT:
            half_spread += int(p["alert_spread_add"])
        ...
        if self.regime == Regime.ALERT:
            vol_scale *= float(p["alert_size_mult"])
        if self.regime == Regime.SHOCK or self.shock_remaining > 0:
