"""M4: j3 regime switch + short-lived reversal branch on sharp trend flips (k4-style de-risk)."""
...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            rev_k = max(1.14, 1.60 * max(self.vol_ema, 0.27))
            against_long = net_inv > 1.38 and move <= -rev_k
            against_short = net_inv < -1.38 and move >= rev_k
            trend_flip = (
                prev_trend * self.trend < 0
                and abs(prev_trend) > 0.095
                and abs(self.trend) > 0.072
            )
            counter_trend = (
                prev_trend * move < 0
                and abs(move) >= rev_k
                and abs(prev_trend) > 0.112
            )
