"""O3 round13: m2 calm-depth + short-lived reversal (widen + cut toxic side), gated to reduce false triggers."""
...
class Strategy(BaseStrategy):
    ...
    _L3_FRAC = 0.21
    _MIN_DEPTH = 0.5
    # Reversal lite: stricter move/inventory gates, 2-step window, no passive depth while active
    _REV_STEPS = 2
    _REV_INV_MIN = 1.52
    _REV_K_BASE = 1.24
    _REV_K_VOL = 1.74
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            rev_k = max(self._REV_K_BASE, self._REV_K_VOL * max(self.vol_ema, 0.27))
            against_long = net_inv > self._REV_INV_MIN and move <= -rev_k
            against_short = net_inv < -self._REV_INV_MIN and move >= rev_k
            trend_flip = (
                prev_trend * self.trend < 0
                and abs(prev_trend) > 0.108
                and abs(self.trend) > 0.082
            )
            counter_trend = (
                prev_trend * move < 0
                and abs(move) >= rev_k * 1.04
                and abs(prev_trend) > 0.118
            )
            if against_long or against_short or trend_flip or counter_trend:
                self.reversal_remaining = max(self.reversal_remaining, self._REV_STEPS)
        ...
        if self.reversal_remaining > 0:
            if net_inv > 0.82:
                bid_size *= 0.18
                my_bid = max(1, int(round(fair - (bid_half + 2))))
            elif net_inv < -0.82:
                ask_size *= 0.18
                my_ask = min(99, int(round(fair + (ask_half + 2))))
            if my_bid >= my_ask:
                return actions
