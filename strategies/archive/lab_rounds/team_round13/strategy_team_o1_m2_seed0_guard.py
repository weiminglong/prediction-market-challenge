"""m2 j3 calm-depth + stricter gate + shallower passive depth when uncertainty rises."""
...
class Strategy(BaseStrategy):
    ...
    # Stricter calm-depth gate vs m2
    _DEPTH_REGIME_MAX = 0.30
    _DEPTH_VOL_MAX = 0.54
    _DEPTH_TOX_ABS_MAX = 0.86
    _DEPTH_TREND_ABS_MAX = 0.54
    _L2_EXTRA = 4
    _L3_EXTRA = 7
    _L2_FRAC = 0.36
    _L3_FRAC = 0.21
    _MIN_DEPTH = 0.5
    # Uncertainty scaling for L2/L3 (model_vol + vol_ema)
    _UNC_MV_REF = 2.45
    _UNC_VOL_REF = 0.62
    ...
    def _depth_stress(self, regime_raw: float, model_vol: float) -> float:
        r = regime_raw / max(1e-9, self._DEPTH_REGIME_MAX * 0.92)
        mv = model_vol / max(1e-9, self._UNC_MV_REF)
        ve = self.vol_ema / max(1e-9, self._UNC_VOL_REF)
        return min(1.0, max(r, mv, ve))
    ...
    def on_step(self, state: StepState):
        ...
        stress = self._depth_stress(regime_raw, model_vol)
        dscale = 1.0 - 0.24 * stress
        fscale = 1.0 - 0.11 * stress
        l2e = max(2, int(round(self._L2_EXTRA * dscale)))
        l3e = max(4, int(round(self._L3_EXTRA * dscale)))
        ms = self._MIN_DEPTH
        l2_frac = self._L2_FRAC * fscale
        l3_frac = self._L3_FRAC * fscale

        bid_L2 = max(1, my_bid - l2e)
        ask_L2 = min(99, my_ask + l2e)
        bid_sz2 = max(ms, bid_size * l2_frac) if bid_size > 0 else 0.0
        ask_sz2 = max(ms, ask_size * l2_frac) if ask_size > 0 else 0.0
        ...
        bid_sz3 = max(ms, bid_size * l3_frac) if bid_size > 0 else 0.0
        ask_sz3 = max(ms, ask_size * l3_frac) if ask_size > 0 else 0.0
