"""o4 + realized-variance throttle: scales L2/L3 depth size during mid bursts (L1 unchanged)."""
...
class Strategy(BaseStrategy):
    ...
    # Realized variance (squared mid move, ticks^2); throttles L2/L3 only
    _RV_EMA_DECAY = 0.90
    _RV_REF = 1.85
    _RV_THROTTLE_K = 0.42
    ...
    def _depth_rv_scale(self) -> float:
        excess = max(0.0, self.rv_ema - self._RV_REF)
        return 1.0 / (1.0 + self._RV_THROTTLE_K * excess)
    ...
    def _depth_l23_skew(self, net_inv: float) -> tuple[float, float, float, float]:
        """L2/L3: inventory skew + extra size/offset when trend aligns with unwind."""
    ...
    def on_step(self, state: StepState):
        ...
        if self.prev_bid is not None and self.prev_ask is not None:
            ...
            r2 = move * move
            w = 1.0 - self._RV_EMA_DECAY
            self.rv_ema = self._RV_EMA_DECAY * self.rv_ema + w * r2
        ...
        bid_sz2 = max(ms, bid_size * self._L2_FRAC * bsz_m * rv_s) if bid_size > 0 else 0.0
        ask_sz2 = max(ms, ask_size * self._L2_FRAC * asz_m * rv_s) if ask_size > 0 else 0.0
        ...
        bid_sz3 = max(ms, bid_size * self._L3_FRAC * bsz_m * rv_s) if bid_size > 0 else 0.0
        ask_sz3 = max(ms, ask_size * self._L3_FRAC * asz_m * rv_s) if ask_size > 0 else 0.0
