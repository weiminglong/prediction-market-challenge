"""P3 punch: o4 core + modestly stronger calm-depth on trend×inventory-aligned side (hard-capped)."""
...
class Strategy(BaseStrategy):
    ...
    # Trend × inventory alignment (calm depth): slightly punchier on favorable side
    _DEPTH_TREND_TANH_SCALE = 0.52
    _DEPTH_TREND_SIZE_COEF = 0.072
    _DEPTH_TREND_OFF_COEF = 0.060
    _DEPTH_COMB_SZ_MIN = 0.76
    _DEPTH_COMB_SZ_MAX = 1.28
    _DEPTH_COMB_OFF_MIN = 0.81
    _DEPTH_COMB_OFF_MAX = 1.19
    # Hard ceiling after favorable boost (cannot exceed even if chained mults spike)
    _DEPTH_HARD_SZ_CAP = 1.285
    ...
    def _depth_l23_skew(self, net_inv: float) -> tuple[float, float, float, float]:
        ...
        cap = self._DEPTH_HARD_SZ_CAP
        bid_sz = min(bid_sz, cap)
        ask_sz = min(ask_sz, cap)
    ...
    def on_step(self, state: StepState):
        ...
        bid_sz2 = max(ms, bid_size * self._L2_FRAC * bsz_m) if bid_size > 0 else 0.0
        ask_sz2 = max(ms, ask_size * self._L2_FRAC * asz_m) if ask_size > 0 else 0.0
        ...
        bid_sz3 = max(ms, bid_size * self._L3_FRAC * bsz_m) if bid_size > 0 else 0.0
        ask_sz3 = max(ms, ask_size * self._L3_FRAC * asz_m) if ask_size > 0 else 0.0
